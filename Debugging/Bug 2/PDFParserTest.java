...
//        assertTrue(content.contains("wherever possible"));
    ...
    @Test
    public void test_fussball_Pdf_Parsing() throws Exception {
        Parser parser = new AutoDetectParser(); // Should auto-detect!
        Metadata metadata = new Metadata();

        InputStream stream = PDFParserTest.class.getResourceAsStream(
                "/test-documents/fussball.pdf");

        String content = getText(stream, parser, metadata);

        System.out.println(content);
        
        assertTrue(content.contains("SCHULE/KINDERGRUPPE"));
    }
    
    
    @Test
    public void test_fussballPdf_Parsing() throws Exception {
        Parser parser = new AutoDetectParser(); // Should auto-detect!
        Metadata metadata = new Metadata();

        InputStream stream = PDFParserTest.class.getResourceAsStream(
                "/test-documents/RG_AMF_Livre_IV_8091_1.pdf");
