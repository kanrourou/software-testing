...
public class Mp3ParserTest {
    ...
    /**
     * Test that with only Save-the-World-Knife-Party-Remix.mp3 tags.   
     */
    @Test
    public void testMp3ParsingSavetheWorldKnifePartyRemix() throws Exception {
        Parser parser = new AutoDetectParser(); // Should auto-detect!
        ContentHandler handler = new BodyContentHandler();
        Metadata metadata = new Metadata();

        InputStream stream = Mp3ParserTest.class.getResourceAsStream(
                "/test-documents/Save-the-World-Knife-Party-Remix.mp3");
        try {
            parser.parse(stream, handler, metadata, new ParseContext());
        } finally {
            stream.close();
...
//        assertEquals("audio/mpeg", metadata.get(Metadata.CONTENT_TYPE));
//        assertEquals("Save the World (Knife Party Remix)", metadata.get(TikaCoreProperties.TITLE));
//
//        String content = handler.toString();
//        assertTrue(content.contains("Save the World (Knife Party Remix)"));
//        assertTrue(content.contains("Swedish House Mafia"));
//        assertTrue(content.contains("Metrojolt.com"));
//        assertTrue(content.contains("The Metroplitan Jolt"));
//        
//        assertEquals("44100", metadata.get("samplerate"));
//        assertEquals("2", metadata.get("channels"));
//        checkDuration(metadata, 2);
